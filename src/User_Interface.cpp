    ...
    vector <string> const& locusNames, string const& dataType)
{
    ...
    while (!done) {
        ...
        if (checkCharValue(userChoice,'n') || checkCharValue(userChoice,'b')) {
            ...
            writeNexus(numTaxa, numChar, taxonNames, fileName, taxaAlignment, includedLocusRanges, locusNames, dataType);
            done = true;
        }
    ...
    vector <string> const& locusNames, string const& dataType)
{
    ...
    // TODO: use real datatype, get rid of interleave
    
    outFile << "#NEXUS" << endl << endl
    << "[ *** File generated by Decisivator v" << version << " ***]" << endl
    << endl
    << "Begin data;" << endl
