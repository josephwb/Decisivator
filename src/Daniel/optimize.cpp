...
int optimizeDecisivnessGA (vector < vector <int> > & data, vector < vector <int> > const& legal, int maxGenes)
{
	...
	// Function currently errors out when too many genes initially present. This should be changed to allow genes to be removed.
	...
	while (!done){
		...
		if (randmu < mutationProbability){
			...
			while (randx == worst){ randx = (int)(rand()%populationSize);}
...
// Function to mutate individual. Currently this function calculates number of legal positions each time. This can be avoided by calculating this once and passing these values to this function. add and remove are flags to determine if gene is added and/or removed.
void mutate ( vector < vector <int> > & data, vector < vector <int> > const& legal, bool add, bool remove )
{
	int numTaxa = data.size();
	int numLoci = data[0].size();
	...
	int radd=0;
	if (add) { if (!count0){cout << "All legal loci full, cannot add" << endl; return;}; radd = rand() % count0;}
	// select random currently present gene to remove
	int rremove=0;
	if (remove) { if (!count1){cout << "All legal loci empty, cannot remove" << endl; return;}; rremove = rand() % count1;}
	...
	for (int i=0; i<numTaxa; i++){ 
		for (int j=0; j<numLoci; j++){ 
			...
			if (legal[i][j]){
				...
				if (remove) {if (count1 == rremove){ data[i][j] = 0; rremove = -1;}}
				if (add) {if (count0 == radd){ data[i][j] = 1; radd = -1;}}
...
void xover ( vector < vector <int> > & recipient, vector < vector <int> > const& donor, vector < vector <int> > legal, int maxGenes)
{
	...
	if (numAdd > 0){for (int i=0; i<numAdd; i++){mutate(recipient, legal, 1, 0);}}
	if (numAdd < 0){for (int i=numAdd; i<0; i++){mutate(recipient, legal, 0, 1);}}
...
double calcFit( vector < vector <int> > const& individual)
{
	...
	int sumT[numTaxa];
	int sumL[numLoci];
	for (int i=0; i<numTaxa; i++){sumT[i]=0;}
	...
	double prodL = 1;
	for (int i=0; i<numTaxa; i++){
		if (sumT[i]>=1) prodT=prodT*sumT[i];
	}
	for (int i=0; i<numLoci; i++){
		if (sumL[i]>=1) prodL=prodL*sumL[i];
	}
	double fit = 0;
	//cout << "prodL: " << prodL << " prodT: " << prodT << endl; 
	if ((prodT+prodL)>0) {fit = prodT/(prodT+prodL);}
	if ((fit>1) || (fit<0)) {cout << " fitness error " << endl; return(1);}
...
// 12/10/2011
// Hill climbing approach similar to stepAdd. In this function genes are added randomly until maxGenes reached. The genes are then sequentially moved from current location to best location one by one. This process repeats untill no moves increases fitness. Can be stochastic result, depends on starting condition (initialized randomly). This could result in an infinite loop. TODO: set maximum number of loops
int stepReplace (vector < vector < int > > & data, vector < vector < int > > const& legal, int maxGenes)
{
